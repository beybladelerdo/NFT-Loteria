<script lang="ts">
  ...
  let tablaSortKey = $state<
    "gamesPlayed" | "earningsICP" | "earningsCkBTC" | "earningsGLDT"
  >("earningsICP");
...
<section class="arcade-panel p-4 sm:p-6">
  <div class="mb-4 sm:mb-6">
    ...
    <h2
      class="mt-3 text-2xl sm:text-3xl font-black uppercase text-[#F4E04D] arcade-text-shadow"
    >
      TOP EARNING TABLAS
    </h2>
  ...
  <div class="mb-4 sm:mb-6 arcade-panel-sm p-4">
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      <div class="md:col-span-1">
        <label
          for="tabla-search"
          class="block text-xs sm:text-sm font-black text-[#F4E04D] uppercase mb-2"
        >
          Search by Tabla ID
        </label>
        <input
  ...
  {:else if filteredTablaStats.length === 0}
    <div class="arcade-panel-sm py-6 text-center text-[#C9B5E8] text-sm">
      {tablaSearch
        ? "No tablas found matching that ID."
        : "No tabla stats yet. Play some games to see earnings!"}
    </div>
  {:else}
    <div
    ...
    >
      {#each filteredTablaStats as t}
        ...
        >
          <div
            class="relative mb-2 overflow-hidden rounded border-2 border-black aspect-[569/1000]"
          >
            <img
            ...
            />
            <div
              class="absolute top-1 right-1 bg-[#F4E04D] text-[#1a0033] border-2 border-black px-2 py-0.5 text-[10px] font-black shadow-[2px_2px_0px_rgba(0,0,0,1)]"
            >
              #{t.tablaId}
            </div>
          </div>
          ...
          <div class="mb-2">
            <p
              class="text-[10px] sm:text-xs text-[#C9B5E8] uppercase font-black text-center"
            >
          ...
          <div class="grid grid-cols-3 gap-1 text-[9px] sm:text-[10px]">
            <div
              class="bg-[#25004d] border border-black px-1 py-1 rounded text-center"
            >
