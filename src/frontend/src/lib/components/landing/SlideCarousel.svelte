<script lang="ts">
  ...
  function pickWindow<T>(arr: T[], s: number, n: number): T[] {
    ...
    const out: T[] = [];
    for (let i = 0; i < Math.min(n, arr.length); i++) out.push(arr[(s + i) % arr.length]);
    return out;
  }

  $: current = pickWindow(items, start, Math.min(windowSize, items.length || 0));
...
{#if title}
  <div class="mb-3 flex items-center justify-between">
    <h3 class={`text-lg font-semibold ${dark ? 'text-white' : 'text-black'}`}>{title}</h3>
    <span class={`text-xs ${dark ? 'text-white/60' : 'text-black/50'}`}>{items.length} items</span>
  </div>
{/if}

<div class="relative overflow-hidden rounded-xl">
  <!-- edge fades -->
  <div class={`pointer-events-none absolute left-0 top-0 h-full w-12 bg-gradient-to-r ${dark ? 'from-[#0b0b0b]' : 'from-white'} to-transparent`}></div>
  <div class={`pointer-events-none absolute right-0 top-0 h-full w-12 bg-gradient-to-l ${dark ? 'from-[#0b0b0b]' : 'from-white'} to-transparent`}></div>
