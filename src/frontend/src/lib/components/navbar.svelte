...
<header class="flex justify-between items-center p-4 bg-black bg-opacity-30">
  <div class="flex items-center">
    <a href="/" class="flex items-center">
      <div class="w-10 h-10 rounded-full bg-white flex items-center justify-center text-purple-900 font-bold">LC</div>
  ...
  <div class="hidden md:flex items-center space-x-4">
    {#if authStore.value.isAuthenticated}
      ...
      <div class="flex items-center ml-2">
        <div class="text-white bg-white bg-opacity-20 rounded-full px-3 py-1 mr-2 flex items-center">
          <!--<User size={16} class="mr-1" />-->
          {formatPrincipal(authStore.value.identity?.getPrincipal().toString() ?? null)}
        </div>
        <a href="/wallet" class="bg-white bg-opacity-20 hover:bg-opacity-30 text-white rounded-full px-4 py-2 transition flex items-center">
  ...
  <!-- Mobile menu -->
  {#if mobileMenuOpen}
    <div class="absolute top-16 left-0 right-0 bg-gradient-to-b from-purple-900 to-fuchsia-800 p-4 z-50 md:hidden">
      {#if authStore.value.isAuthenticated}
